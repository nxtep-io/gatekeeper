...
export default class OAuthClientController {
  ...
  public static async create(req: BaseRequest, res: BaseResponse) {
    ...
    if (client) {
      ...
      throw new HttpError("Could not create OAuth Client, unknown error", HttpCode.Server.INTERNAL_SERVER_ERROR, {
        client
      });
    }
  }
  ...
  public static async updateClient(req: BaseRequest, res: BaseResponse) {
    ...
    res.success(await client.update({ $set: changes }, { runValidators: true, new: true }));
  }
