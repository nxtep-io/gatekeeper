...
import { OAuthAccessToken, OAuthClient } from "../models";
...
export default class AnalyticsController {
  ...
  static async getDevicesStats(req, res) {
    ...
    const browser = await OAuthAccessToken.aggregate([
      match,
    ...
    res.success({
      ...
      browser: browser.map(b => ({ [b._id]: b.count })).reduce((aggr, next) => ({ ...aggr, ...next }), {}),
      // Return OS as a hash map
      os: os.map(b => ({ [b._id]: b.count })).reduce((aggr, next) => ({ ...aggr, ...next }), {})
    });
  }
}
